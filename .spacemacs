...
If EXTERNAL is double prefix, browse in new buffer."
  ...
  (spacemacs/set-leader-keys "wm" 'treemacs-delete-other-windows)
...
in the other window."
  ...
  (use-package treemacs
    ...
    (progn
      (defun my/treemacs-ignored-predicates (file _)
        "Ignored predicates."
        (s-matches? (rx bol
                        (or ".gitignore"  ".project" ".settings" ".classpath"
                            ".meghanada" ".lein-env" ".lein-failures" ".hgignore"
                            ".lein-repl-history")
                        eol)
                    file))
  ...
  ;; (define-key evil-normal-state-map (kbd "-") 'vinegar)
