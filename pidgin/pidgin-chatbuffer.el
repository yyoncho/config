...
(defconst pidgin-chat-line-dilimeter (propertize "----\n") '(read-only t))
  ...
  (let ((inhibit-read-only t)
    ...
    (unless (eq (point) beg)
      (let ((end (point-marker)))
        ...
        (write-region beg end (concat pidgin-messenger-directory pidgin-chating-with) t 'no-echo)
...
(defun pidgin-chat-with (protocol)
  (interactive)
  (let* ((user (let ()
                 (pidgin-protocol-user-name
                  (completing-read  "chat with: " (pidgin-user-list protocol))
                  protocol)))
