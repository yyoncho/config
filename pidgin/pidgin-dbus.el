...
(defun pidgin-recieve-signal (account sender text conversation flags)
  (let* ((protocol (car (rassoc account pidgin-accounts)))
    ...
    (pidgin-chat-recieve
     ...
     (if (s-equals? message "<img id=\"2\">") "*thumb up*" (xml-substitute-special message)))))

(defun pidgin-receive-my-signal (& args)
  (message (list-length args)))


(defun pidgin-parse-message (message)
  (message "%s" (format "from pidgin: '%s'" message))
  (with-temp-buffer
    (insert message)
